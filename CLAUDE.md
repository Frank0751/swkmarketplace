...
All templates live in `lib/email/brevo.ts`; admin alerts go to `ADMIN_NOTIFICATION_EMAIL`
(default info@swkghana.org). Escape anything a user typed before it goes into an email.
...
| Problem reported (disputed) | Buyer, vendor, admin |
| Refund issued | Buyer |
| Payout released | Vendor |
| Vendor application submitted / resubmitted | Admin |
...
8. **The database doesn't trust the browser (migration 007).** Guard triggers stop users changing
   ...
   `npm run test:db` after any migration change: it replays every migration in an in-memory
...
- Supabase project: zxxzmqrxvmkscibfhpjn (`qaen86pl` is the Sanity project behind swkghana.org)
